...
// DRIVE_FOLDER_DOCUMENTS=<folderId>
...
function folderIdFor(folder) {
  ...
  else if (folder === "star_electronic_documents")
    id = process.env.DRIVE_FOLDER_DOCUMENTS;
  else if (folder === "data")
...
export async function listMedia(folder) {
  ...
  const isDocuments = folder === "star_electronic_documents";

  // List files for this folder type
  const { data } = await drive.files.list({
    q: isDocuments
      ? `'${fid}' in parents and mimeType = 'application/pdf' and trashed = false`
