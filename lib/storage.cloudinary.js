...
export async function listMedia(folder) {
  const cached = getCachedValue(mediaCache, folder);
  if (cached !== CACHE_MISS) {
    return cached;
  }
  ...
  setCachedValue(mediaCache, folder, ordered, MEDIA_CACHE_TTL_MS);
  return ordered;
}
