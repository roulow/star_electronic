...
export function formatContactEmail({
  ...
  if (template) {
    // Normalize templates that may have had their mustache braces
    // accidentally split by HTML tags (e.g. "{</a>{date}}") or
    // encoded as entities. Collapse such fragments so replacements work.
    let tpl = template
      .replace(/&#123;/g, "{")
      .replace(/&#125;/g, "}")
      .replace(/\{\s*(?:<\/?[^>]+>\s*)+\{/g, "{{")
...
export function formatConfirmationEmail({ name, subject, template }) {
  ...
  if (template) {
    // Normalize broken mustache fragments and entities before replacement
    let tpl = template
      .replace(/&#123;/g, "{")
      .replace(/&#125;/g, "}")
      .replace(/\{\s*(?:<\/?[^>]+>\s*)+\{/g, "{{")
