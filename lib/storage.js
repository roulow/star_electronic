...
function normalizeMeta(meta) {
  const normalized = {
    ...
    descriptions:
      meta?.descriptions && typeof meta.descriptions === "object"
        ? meta.descriptions
        : {},
    contentIndex:
      meta?.contentIndex && typeof meta.contentIndex === "object"
        ? meta.contentIndex
        : {},
  ...
  return {
    ...
    contentIndex: normalized.contentIndex,
...
export async function listMedia(folder, options = {}) {
  if (process.env.STORAGE?.toLowerCase() === "cloudinary") {
    ...
    return mod.listMedia(folder, options);
  }
...
export async function readMediaMeta(folder, options = {}) {
  if (process.env.STORAGE?.toLowerCase() === "cloudinary") {
    const mod = await import("./storage.cloudinary.js");
    return normalizeMeta(
      (await mod.readJson(`${folder}.meta.json`, options)) || null,
    );
  }
  if (process.env.STORAGE?.toLowerCase() === "drive") {
    const mod = await import("./storage.drive.js");
  ...
  return readMeta(folder);
}

export async function writeMediaMeta(folder, meta) {
  if (process.env.STORAGE?.toLowerCase() === "cloudinary") {
    const mod = await import("./storage.cloudinary.js");
    return mod.writeJson(`${folder}.meta.json`, meta);
  }
  if (process.env.STORAGE?.toLowerCase() === "drive") {
    const mod = await import("./storage.drive.js");
    return mod.writeJson(`${folder}.meta.json`, meta);
  }
  return writeMeta(folder, meta);
}

export async function getMediaVersion(folder, options = {}) {
  const meta = await readMediaMeta(folder, options);
  const value = Number(meta?.version);
  return Number.isFinite(value) ? value : 0;
}

export async function bumpMediaVersion(folder) {
  const meta = await readMediaMeta(folder, { bypassCache: true });
  const version = Date.now();
  await writeMediaMeta(folder, { ...meta, version });
  return version;
}
...
export async function uploadFiles(folder, blobs) {
  ...
  for (const blob of blobs) {
    ...
    uploaded.push({
      id: name,
      name,
      url: `/uploads/${folder}/${encodeURIComponent(name)}`,
    });
  }
  return uploaded;
}
